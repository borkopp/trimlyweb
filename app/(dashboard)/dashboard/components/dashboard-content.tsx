...
export default async function DashboardContent({
  ...
  try {
    ...
    console.log(
      `[DashboardContent] Fetched ${appointmentsData.length} initial appointments for barbershop ${barbershopId}`
    );
  } catch (error) {
    console.error(
      "[DashboardContent] Error fetching initial appointments:",
      error
    );
      ...
      <main className="grid flex-1 items-start gap-2 p-10 md:gap-8 lg:grid-cols-3 xl:grid-cols-3">
        <div className="grid auto-rows-max items-start gap-4 md:gap-8 lg:col-span-2">
          ...
          <Suspense fallback={<div>Loading stats...</div>}>
            <DashboardStats
              userId={user.id}
              barbershopId={barbershopIdString}
            />
          </Suspense>
