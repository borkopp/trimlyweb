...
type TDialogOverlayProps = ComponentPropsWithoutRef<
  typeof DialogPrimitive.Overlay
> & { disableAnimation?: boolean };

const DialogOverlay = forwardRef<TDialogOverlayRef, TDialogOverlayProps>(
...
const DialogClose = forwardRef<TDialogCloseRef, TDialogCloseProps>(
...
type TDialogContentProps = ComponentPropsWithoutRef<
  typeof DialogPrimitive.Content
> &
  VariantProps<typeof dialogVariants> & { disableAnimation?: boolean };

const DialogContent = forwardRef<TDialogContentRef, TDialogContentProps>(
  ({ className, children, size, disableAnimation = false, ...props }, ref) => {
        ...
        <DialogOverlay disableAnimation={disableAnimation}>
          <div className="flex w-full items-center justify-center px-2">
            <DialogPrimitive.Content
              ...
              {...props}
            >
              <div className="flex flex-col gap-4 rounded-[10px] border border-b-primary dark:border-b-secondary">
                {children}
              </div>
            </DialogPrimitive.Content>
          </div>
...
const DialogHeader = forwardRef<TDialogHeaderRef, TDialogHeaderProps>(
  ({ className, children, ...props }, ref) => (
    <header
      ref={ref}
...
const DialogTitle = forwardRef<TDialogTitleRef, TDialogTitleProps>(
...
type TDialogDescriptionProps = ComponentPropsWithoutRef<
...
const DialogBody = forwardRef<TDialogBodyRef, TDialogBodyProps>(
  ({ className, children }, ref) => {
    return (
...
const DialogFooter = forwardRef<TDialogFooterRef, TDialogFooterProps>(
  ({ className, children }, ref) => {
    return (
      <footer
        ref={ref}
        className={cn(
          "-mx-4 flex flex-col-reverse gap-2 border-t px-4 pt-4 sm:flex-row sm:justify-end [&>button]:w-full",
          className
        )}
      >
        {children}
