...
type AppointmentWithNullableClient =
...
export default function AppointmentRow({ appointment, loggedInUserId }: Props) {
  ...
  const isCancelled =
    appointment.is_cancelled || appointment.is_cancelled_by_barber;
  ...
  if (isWalkIn) {
    ...
    customerDetails = (
      <div className="text-xs text-muted-foreground italic">Walk-in/Phone</div>
    );
  ...
  const handleRowClick = () => {
    ...
    if (appointment.client) {
      ...
      console.warn(
        "Selecting walk-in appointment, context might expect client data:",
        appointment.id
      );
      setSelectedAppointment(appointment as AppointmentWithClient); // CAUTION: Casting walk-in
    } else {
      console.warn(
        "Cannot select appointment row without client data or walk-in info:",
        appointment.id
      );
