...
export async function createTenantClient() {
  ...
  const domain = isDevelopment
    ? "localhost"
    : host.split(":")[0].split(".").slice(-2).join(".");
...
export class TenantQueryBuilder {
  ...
  async getAppointments(filters?: any) {
    const query = this.from("appointments").select(`
        *,
  ...
  async getClients() {
    return this.from("profiles")
      ...
      .not(
        "id",
        "in",
        `(SELECT user_id FROM barbers WHERE barbershop_id = ${this.tenantId})`
      )
      .order("full_name");
  }
}
...
export async function createTenantQueryBuilder() {
  ...
  return new TenantQueryBuilder(
    client,
    tenant.isMainDomain || tenant.id <= 0 ? undefined : tenant.id
  );
}
